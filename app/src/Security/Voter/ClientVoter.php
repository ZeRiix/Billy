...
class ClientVoter extends Voter
{
	...
	{
		return in_array($attribute, [self::CREATE, self::UPDATE, self::VIEW]) &&
			($subject instanceof Organization || $subject instanceof Client);
	}
	...
	protected function voteOnAttribute(string $attribute, mixed $subject, TokenInterface $token): bool
	{
		...
		if ($attribute === self::VIEW) {
			...
			return $this->roleRepository->checkPermissionOnOrganization(
				$user,
				$organization,
				"manage_client"
			);
